...
def _git_status():
    try:
        commit = subprocess.check_output(
            ["git", "show", "--date=short", "--format=format:%ad %h"]).decode().strip()
...
def help(bot, event, cmd=None, *args):
    ...
    if not cmd or (cmd == "impersonate" and event.user.id_.chat_id in admins_list):
        ...
        help_lines.append(
            "<b>{}</b>: {}".format(command_fn.__name__, _docstring))
...
def optout(bot, event, *args):
    ...
    * /bot optout all - clears per-conversation opt-out and forces global optout"""
    ...
    if type_optout is list:
        ...
        raise TypeError(
            'unrecognised {} for optout, value={}'.format(type_optout, optout))
...
def version(bot, event, *args):
    ...
    version_info.append(
        _("Bot Version: **{}**").format(__version__))  # hangoutsbot
    if commit:
        version_info.append(_("Git: **{}**").format(commit))
    version_info.append(
        _("Python Version: **{}**").format(sys.version.split()[0]))  # python
    ...
    if event.user.id_.chat_id in admins_list:
        ...
        for module_name in modules:
            try:
                ...
                version_info.append(
                    _("* {} **{}**").format(module_name, _module.__version__))
...
def unknown_command(bot, event, *args):
    ...
    if not (config_silent or tagged_silent):
        ...
        yield from bot.coro_send_message(event.conv,
                                         _('{}: Care Culo').format(event.user.full_name))
